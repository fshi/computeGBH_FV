...
descFeature::descFeature(
		...
		_2chnls(chnl2)
{
	_iv = new IntegralVideo(_nbins, _fullOri, _2chnls);
	...
	_feaSz = _rtSz * (1 + _numParts.x * _numParts.y * _numParts.z);  //*2 is for optical flow u and v
	if(_2chnls)
		_feaSz *= 2;
}
...
bool descFeature::preProcessor(const string& fName, int stFrame, int endFrame)
{
	if (_iv)
		delete _iv;
	_iv = new IntegralVideo(fName, _nbins, _fullOri,_rt2ps, _reSzVideo,_2chnls, stFrame, endFrame);
	return _iv->hasIv();
}
...
*/	
	if(_2chnls)
	{
		...
		opcvL2Norm(ft+_rtSz, _feaSz-_rtSz);
